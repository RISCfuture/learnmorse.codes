<template>
  <div class="test-symbols">
    <transition appear mode="out-in" name="in-grow-out-grow-linear">
      <start-typing v-if="startTyping" key="startTyping" />
      <field
        v-else
        key="test"
        :lesson="lesson"
...
const startTyping = ref(!isMobile)
const pencilsDown = ref(false)

function onFinishing() {
  cancelTimers()
  pencilsDown.value = true
  addTimer(delayBeforeScoring, () => (pencilsDown.value = false))
}
...
function reset() {
  cancelTimers()
  startTyping.value = !isMobile
  addTimer(delayBeforeTyping, () => (startTyping.value = false))
}
